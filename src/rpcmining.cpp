...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Scicoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Scicoin is downloading blocks...");
    ...
    if (params.size() == 0)
    {
	    ...
	    // Scicoin: the key is the wallet's default key, always
	    pblocktemplate = CreateNewBlock(pwalletMain->vchDefaultKey);
            if (!pblocktemplate)
	...
	// Scicoin: checkwork doesn't need to "keep a key" now that we always pay to the wallet's default key
...
Value getblocktemplate(const Array& params, bool fHelp)
{
        ...
        (nTransactionsUpdated != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        //pblocktemplate = CreateNewBlock(*pMiningKey);
	// Scicoin: mining key is the main wallet's default key, always
	pblocktemplate = CreateNewBlock(pwalletMain->vchDefaultKey);
        if (!pblocktemplate)
            throw JSONRPCError(RPC_OUT_OF_MEMORY, "Out of memory");

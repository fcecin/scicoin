...
// Scicoin 0.00001000 fee seems better (it is per kb)
mpq CTransaction::nMinTxFee = mpq("1000/1");  // Override with -mintxfee
...
bool CTxOut::IsDust() const
{
    ...
    // In Scicoin, dust is anything under 1 satoshi, so no.
    return false;
}
...
/*
  // scicoin hack: budget is always empty so this is always a NO-OP
...
/*
  // scicoin hack: dead code, single callsite deleted
...
// scicoin hack: all irrelevant now
//
//mpq static GetInitialDistributionAmount(int nHeight)
//{
// scicoin hack: no intial distribution
//  return mpq("0");
//}
//
...
// scicoin hack: 1:1 reward for the difficulty plus fees
#include "bitcoinrpc.h"
// paidto: the recipient of this block's (the block we're getting the value of) coinbase reward
// pindexPrev: the CBlockIndex of the previous block (so we can see to whom the coinbase of THAT went, and its prev, and its prev...)
//mpq static GetBlockValue(unsigned int nBits, int nHeight, const mpq& nFees, CPubKey& paidto, CBlockIndex* pindexPrev)
// better to just compare the CScript directly
mpq static GetBlockValue(unsigned int nBits, int nHeight, const mpq& nFees, CScript& paidto, CBlockIndex* pindexPrev)
{
  // 1 scicoin per petachop (quadrillion Coin Hash OPerations)
  // At difficulty dDiff=1 the code requires 2^32 CHOPs per block on average, hence the constant below.

  int nShift = (nBits >> 24) & 0xff;
  mpq dup = 0x0000ffff;
  mpq ddn = nBits & 0x00ffffff;
  mpq dDiff = dup / ddn;
  while (nShift < 29) {
    dDiff *= 256;
    nShift++;
  ...
  //----- BEGIN Scicoin's exponential reward blocks found by same miner
  
  //==================================================================================================================
  // The ideal Scicoin deployment scenario is one where all scientific computing projects work together to operate
  //  a single scicoin 'mining pool'. When that happens, the network detects that the majority of the mining is 
  //  going to the same operator and we presume that that operator is the scientific community.
  // The goal of Scicoin is to give 'free' coins to the scientific computing project operators, way beyond the
  //  computing power they spend to mine the coins (e.g. up to 10,000 times more if they control ALL the mining).
  // The social contract is that the 'extra' coins that don't correspond to the default mining rate (1 petahash per
  //  coin) are to be given only to those who contribute to scientific computing projects (e.g. find primes or
  //  something) and they should be awarded at approximately the same rate the default mining operation rewards
  //  (i.e. charge the equivalent of 1 petahash of scientific computing work per coin). If that happens then 
  //  the 'value' of Scicoin remains constant, i.e. one coin is worth whatever 1 petahash of proof-of-work is 
  //  worth.
  // Today there's one Bitcoin mining pool that has almost 50% of the computing power of about 18 petahashes 
  //  per second. It is not really impractical to envision all scientific computing projects interested in
  //  scicoin getting together on the same mining pool to mine it. In case we ever want something better, more 
  //  scalable, etc. we can just improve the current solution to something better, which shouldn't be too hard.
  //  But I think the solution in its current form can accomodate a massive amount of project interest before 
  //  the overly-centralized exponential-reward scheme becomes a potential issue.
  // Finally, it is worth remembering that Scicoin is not and will never be considered 'money.' To Scicoin to be
  //  socially (i.e. "legally" etc.) considered 'money' (i.e. information you have to be careful to manipulate
  //  else you can be thrown into a dungeon, persecuted, etc.) the level of social insanity has to be immense
  //  (I would like to say "unprecedented" but sadly there is a substantial amount of precedent in history). If
  //  Scicoin is ever considered 'money' then Hashcash-protected e-mails are money too, and from there on 
  //  any form of information exchange becomes imbued with grave social weight, a vehicle to 'debt', etc. Truly
  //  an unlikely future.
  // And since Scicoin just isn't becoming 'money' in any plausible social future, we can relax and not bother 
  //  too much about whether the single pool operator, on behalf of the scientific community, will have a motivation 
  //  to execute '51% attacks,' or whether we should worry about 'rogue private entities' starting to mine Scicoin 
  //  to reap itself a 10,000 times 'profit' over non-money. All nonsense. Nobody should ever be afraid of any of 
  //  these 'security holes' being exploited. If attacks happen, meh, just make the thing better and ignore the 
  //  effects of the attacks themselves: nobody 'lost' anything, any 'stored value.' It's a game. A toy.
  //==================================================================================================================
  
  int visited = 0;
  int matches = 0;

  // visit up to nOriginalInterval (2016) blocks before the one we're getting the value of.
  const int span = nOriginalInterval;
  
  for (CBlockIndex* pindex = pindexPrev; pindex && pindex->pprev && (visited < span); pindex = pindex->pprev) {

    CBlock block;
    if (!block.ReadFromDisk(pindex)) {
      // FIXME: is this right?
      error("GetBlockValue(): Can't read a block from disk while we should totally be able to."); // I hope this is the right way to log
      assert(false);
      throw;
      exit(1);
    }
    ...
    // check scriptPubKey of coinbase match the given one
    if (block.vtx[0].vout[0].scriptPubKey == paidto)
      ++ matches;
  }

  mpq nReward = dDiff * 4294967296;
 
  // let's keep it simple:
  // greater than 4/5 (~ >80%) : reward x10000
  // not greater than 4/5, but greater than 2/3 (~ 66.6%...80%) : reward x1000
  // not greater than 2/3, but greater than 1/2 (~ 50%...66.6%) : reward x100
  ...
  nReward /= 10000000;  // 2^32 operations per diff ---> 429.4967296 satoshis per diff
                        // with majority mining power that's 4294967.296 satoshis per diff (~0.043 scicoins per diff)
                        // with bitcoin's current ~2 billion diff, a majority-mined block gives ~100 million coins per block
  
  //----- END Scicoin's exponential reward blocks found by same miner
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // Scicoin: we need to reverse-engineer the CPubKey from the coinbase transaction to be able to determine
    //  the block's value. (we ARE using CPubKeys, right? not pubkey hashes (payment addrs...))
/*
    mpq qAllowedCoinbaseValue = mpq("0");
    vector<vector<unsigned char> > vSolutions;
    txnouttype whichType;
    if (!Solver(vtx[0].vout[0].scriptPubKey, whichType, vSolutions))
      return state.DoS(100, error("ConnectBlock(): Solver() failed for coinbase's scriptPubKey, which should never ever happen."));
    if (whichType != TX_PUBKEY) {
      return state.DoS(100, error("ConnectBlock(): Solver() gave us something different than TX_PUBKEY for the coinbase transaction, which should never ever happen."));
    } else {
      CPubKey coinbasePaysToThisPubKey = vSolutions[0];
      qAllowedCoinbaseValue = GetBlockValue(pindex->nBits, pindex->nHeight, nFees, coinbasePaysToThisPubKey, pindex->pprev);
    }
*/
    // Better: take the CScript to compare and STFU
    mpq qAllowedCoinbaseValue = GetBlockValue(pindex->nBits, pindex->nHeight, nFees, vtx[0].vout[0].scriptPubKey, pindex->pprev);
...
// Scicoin: nope: pay always to the wallet's default key
//CBlockTemplate* CreateNewBlock(CReserveKey& reservekey)
//CBlockTemplate* CreateNewBlock(CWallet* pwallet)
CBlockTemplate* CreateNewBlock(CPubKey& pubkey)
{
    // Scicoin: if the given key to receive block coinbase payout is not valid
	...
	// no budgets anywhere, so these are no-ops
	//
        //std::map<CTxDestination, mpq> mapBudget;
	//
	...
	// Scicoin: the key is the given wallet's default key
        //CPubKey pubkey;
	...
	*/
	...
	// Scicoin: resize(1) unnecessary after we take the budget stuff out, vtx[0].vout[0] is already the single element
	...
	*/
	...
	unsigned int next_work_required = GetNextWorkRequired(pindexPrev, pblock);

	// Scicoin
        //mpq nBlockReward = GetBlockValue(next_work_required, nHeight, nFees, pubkey, pindexPrev);// - nBudgetPaid;
	// better for the other callsite (and the proc impl) if we give the expected scriptPubKey in blocks instead of the pubkey
	mpq nBlockReward = GetBlockValue(next_work_required, nHeight, nFees, pblock->vtx[0].vout[0].scriptPubKey, pindexPrev);// - nBudgetPaid;
        ...
        pblock->nBits          = next_work_required;
        pblock->nNonce         = 0;
...
// Scicoin: no need to "keep the key", we pay to the same one always (the default one)
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    // Scicoin: no. we always pay to the wallet's default key
    //
	...
	// scicoin hack: if you want to mine without a connection
	...
	// Scicoin: nope. pay to the wallet's default key
        //auto_ptr<CBlockTemplate> pblocktemplate(CreateNewBlock(reservekey));
	auto_ptr<CBlockTemplate> pblocktemplate(CreateNewBlock(pwallet->vchDefaultKey));
        ...
        loop
        {
		    ...
		    // Scicoin: the blocks we find pay always to the wallet's default key
